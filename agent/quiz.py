...
def _sanitize_prompt_input(text: str, max_len: int = 500) -> str:
    """安全转义用户输入，防止 LLM prompt 注入。
    
    策略：截断 + 转义花括号（阻止 .format() 注入）+ 去掉控制字符
    """
    if not text:
        return ""
    text = text[:max_len]
...
def _ask_llm_safely(messages: list, temperature: float = 0.7,
                    max_tokens: int = 2048, model_kind: str = "flash",
                    fallback: str = "") -> str:
    """安全调用 LLM，捕获所有异常，返回 fallback 而非崩溃"""
    try:
        return chat_sync(messages, temperature=temperature,
                        max_tokens=max_tokens, model_kind=model_kind)
    except Exception:
        return fallback
...
class QuizManager:
    ...
    def _cleanup_expired(self, ttl_hours: int = 2, max_sessions: int = 200):
        """清理过期/完成的会话，防止内存泄漏"""
        now = datetime.now()
        expired = []
        for sid, s in self.sessions.items():
            age = (now - datetime.fromisoformat(s.created_at)).total_seconds()
            if s.finished or age > ttl_hours * 3600:
                expired.append(sid)
        for sid in expired:
    ...
    def generate_questions(
        ...
        """调用 LLM 生成题目（安全版：topic 转义 + 异常兜底）"""
        # 安全：topic 转义防止 prompt 注入
        safe_topic = _sanitize_prompt_input(topic, max_len=200) or "通用编程知识"
        ...
        # 选择 prompt（topic 已安全转义，可安全 .format()）
        if q_type == "mixed":
            prompt = GEN_QUIZ_PROMPT_MIXED.format(
                count=count, difficulty=difficulty, topic=safe_topic,
        ...
        # 安全调用：LLM 挂了不会崩溃
        response = _ask_llm_safely(messages, temperature=0.9, max_tokens=4096,
                                   model_kind="flash")
        if not response:
            return []  # LLM 失败，返回空让上层处理
    ...
    def start_quiz(
        ...
        if not questions:
            raise ValueError("题目生成失败，LLM 返回为空，请稍后重试")
    ...
    def _grade_answer(self, question: Question, user_answer: str) -> Dict:
        ...
        # 填空/简答/编程：LLM 判卷（user_answer 已安全转义防注入）
        safe_answer = _sanitize_prompt_input(user_answer, max_len=2000)
        prompt = GRADE_PROMPT.format(
            question=_sanitize_prompt_input(question.text, max_len=1000),
            q_type=question.q_type,
            correct_answer=_sanitize_prompt_input(question.answer, max_len=500),
        ...
        response = _ask_llm_safely(messages, temperature=0.3, max_tokens=512,
                                   model_kind="flash")
        try:
            text = response.strip()
            if text.startswith("```"):
